...
impl<T> IntoScanner for T
where
    T: IntoIterator,
    T::IntoIter: Scanner,
{
    type IntoScanner = T::IntoIter;
...
impl<'a> Scanner for std::str::Chars<'a> {
    type SaveState = Self;
...
impl<'a> ScannerSlice for std::str::Chars<'a> {
    type Slice = &'a str;
...
impl<'a, T> Scanner for std::slice::Iter<'a, T> {
    type SaveState = Self;
...
impl<'a, T> ScannerSlice for std::slice::Iter<'a, T> {
    type Slice = &'a [T];
